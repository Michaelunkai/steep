...
"@
    ...
    # COMPLETE DOCKER PURGE AND REINSTALL BEFORE REBOOT
    Write-Log "=== COMPLETE DOCKER PURGE AND FRESH REINSTALL ==="
    try {
        Write-Log "Starting complete Docker removal and fresh installation..."
        
        # Step 1: Stop all Docker processes
        Write-Log "Step 1: Stopping all Docker processes..."
        $dockerProcesses = @("Docker Desktop", "dockerd", "docker", "com.docker.service")
        ...
        Write-Log "Step 2: Stopping Docker services..."
        $dockerServices = @("docker", "com.docker.service", "DockerDesktopVM")
        foreach ($service in $dockerServices) {
            Stop-Service -Name $service -Force -ErrorAction SilentlyContinue
        }
        
        # Step 3: Uninstall Docker Desktop via Windows
        Write-Log "Step 3: Uninstalling Docker Desktop..."
        try {
            # Try using Windows uninstaller
            $dockerUninstaller = Get-ChildItem "C:\Program Files\Docker\Docker" -Filter "Docker Desktop Installer.exe" -ErrorAction SilentlyContinue
            if ($dockerUninstaller) {
                Write-Log "Running Docker Desktop uninstaller..."
                Start-Process $dockerUninstaller.FullName -ArgumentList "uninstall", "--quiet" -Wait -ErrorAction SilentlyContinue
            }
            
            # Also try via Programs and Features
            Write-Log "Attempting uninstall via Windows Programs..."
            $uninstallJob = Start-Job {
                Get-Package -Name "*Docker*" -ErrorAction SilentlyContinue | Uninstall-Package -Force -ErrorAction SilentlyContinue
            }
            Wait-Job $uninstallJob -Timeout 120 | Out-Null
            Remove-Job $uninstallJob -ErrorAction SilentlyContinue
            
        } catch {
            Write-Log "Standard uninstall failed, proceeding with manual removal..." "WARN"
        }
        
        # Step 4: Complete Docker data removal
        Write-Log "Step 4: Removing ALL Docker data and files..."
        $dockerPaths = @(
        ...
        # Step 5: Clean Docker registry entries
        Write-Log "Step 5: Cleaning Docker registry entries..."
        $dockerRegPaths = @(
        ...
        Write-Log "Step 6: Downloading Docker Desktop..."
        $dockerUrl = "https://desktop.docker.com/win/main/amd64/Docker%20Desktop%20Installer.exe"
        $dockerInstaller = "$env:TEMP\DockerDesktopInstaller.exe"
        
        try {
            Write-Log "Downloading from: $dockerUrl"
            $downloadJob = Start-Job {
                param($url, $output)
                Invoke-WebRequest -Uri $url -OutFile $output -UseBasicParsing
            } -ArgumentList $dockerUrl, $dockerInstaller
            
            if (Wait-Job $downloadJob -Timeout 300) {
                Receive-Job $downloadJob
                Remove-Job $downloadJob
                Write-Log "Docker Desktop downloaded successfully"
            } else {
            ...
            Write-Log "Download failed, trying alternative method..." "WARN"
            # Try with curl as backup
            curl -L -o "$dockerInstaller" "$dockerUrl" 2>$null
        }
        
        # Step 7: Install Docker Desktop
        Write-Log "Step 7: Installing Docker Desktop..."
        if (Test-Path $dockerInstaller) {
            try {
                Write-Log "Running Docker Desktop installer..."
                $installProcess = Start-Process $dockerInstaller -ArgumentList "install", "--quiet", "--accept-license" -PassThru
                
                # Wait for installation (max 10 minutes)
                $timeout = 600
                $timer = 0
                while (!$installProcess.HasExited -and $timer -lt $timeout) {
                    Start-Sleep 10
                    $timer += 10
                    if ($timer % 60 -eq 0) {
                        Write-Log "Docker installation in progress... ($timer/$timeout seconds)"
                    }
                }
                
                if (!$installProcess.HasExited) {
                    Write-Log "Installation timeout - force killing installer" "WARN"
                    $installProcess.Kill()
                } else {
                    Write-Log "Docker Desktop installation completed"
                }
            ...
            Write-Log "Docker installer not found - installation skipped" "ERROR"
        }
        
        # Step 8: Start Docker Desktop
        Write-Log "Step 8: Starting Docker Desktop..."
        Start-Sleep 10  # Wait for installation to settle
        
        if (Test-Path "C:\Program Files\Docker\Docker\Docker Desktop.exe") {
            Write-Log "Starting Docker Desktop application..."
            Start-Process "C:\Program Files\Docker\Docker\Docker Desktop.exe" -WindowStyle Hidden
            Start-Sleep 30  # Give Docker time to start
            
            # Step 9: Test Docker functionality
            Write-Log "Step 9: Testing Docker functionality..."
            $testAttempts = 0
            $maxAttempts = 6
            $dockerWorking = $false
            
            while ($testAttempts -lt $maxAttempts -and !$dockerWorking) {
                $testAttempts++
                Write-Log "Docker test attempt $testAttempts/$maxAttempts..."
                
                try {
                    $dockerVersion = docker --version 2>$null
                    if ($dockerVersion) {
                        Write-Log "✅ Docker is working! Version: $dockerVersion"
                        $dockerWorking = $true
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            Write-Log "✅ Docker test container ran successfully!"
                        } else {
                            Write-Log "⚠️  Docker test container failed, but Docker is installed" "WARN"
                        }
                    } else {
                        Write-Log "Docker not responding yet, waiting..."
                        Start-Sleep 30
                    }
                } catch {
                    Write-Log "Docker test failed: $_" "WARN"
                    Start-Sleep 30
                }
            }
            
            if (!$dockerWorking) {
                Write-Log "⚠️  Docker may not be fully ready, but installation completed" "WARN"
            }
            
        } else {
            Write-Log "Docker Desktop executable not found after installation" "ERROR"
        }
        
        Write-Log "=== DOCKER FRESH INSTALLATION COMPLETED ==="
        Write-Log "✅ Docker has been completely removed and freshly reinstalled"
        Write-Log "✅ All old data, configurations, and registry entries cleared"
        Write-Log "✅ Fresh Docker Desktop installation ready for use"
        
    } catch {
        Write-Log "Error during Docker reinstall: $_" "ERROR"
    }
...
Write-Host "  • COMPLETE DOCKER FRESH REINSTALL (before reboot)" -ForegroundColor Red
Write-Host "  • Docker Desktop uninstall, cleanup, download, install" -ForegroundColor Red
Write-Host "  • Docker functionality testing and verification" -ForegroundColor Red
...
Write-Host "• Docker has been COMPLETELY REMOVED and FRESHLY REINSTALLED" -ForegroundColor Red
Write-Host "• Fresh Docker installation tested and verified working" -ForegroundColor Red
...
Write-Host "✅ Docker download timeout (5 minutes)" -ForegroundColor Green
Write-Host "✅ Docker install timeout (10 minutes)" -ForegroundColor Green
...
Write-Host "✅ Fresh Docker installation completed and tested!" -ForegroundColor Green
